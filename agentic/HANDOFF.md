...
- Sibling docs in `agentic/`: [`ARCHITECTURE.md`](ARCHITECTURE.md) (**why the code is shaped this way** — the design rationale that the comment rule forbids in source; read it before changing anything whose reason is not obvious, and add to it when you remove an explanation from a comment), [`AUDIT.md`](AUDIT.md) (security/efficiency audit history — read before doing a pre-release audit so you don't re-flag accepted risks), [`IDEAS.md`](IDEAS.md) (prioritized feature backlog with effort estimates and file touch-lists), [`JellyfinPlugin-SpecialToMovie plan.md`](JellyfinPlugin-SpecialToMovie%20plan.md) (**original pre-implementation design doc — has drifted from reality**, see Gotchas below), and `memory/` (standing project conventions, one rule per file — read `memory/MEMORY.md` first).
...
| `Web/specialtomovie.js` | Embedded resource. Progressive enhancement only — matches links by the plugin's own `stm=` marker (**never** by web-client class names; that coupling caused four field defects in 1.0.17.0), renders them as an icon using the row's own idiom, keeps them last in the row, and navigates in-app from a **capture-phase** click handler rather than from the rewritten `href`. Covered by [`agentic/tools/webclient-harness`](tools/webclient-harness/); every design decision in it is written up in [`ARCHITECTURE.md`](ARCHITECTURE.md). |

**How the link is styled.** It is not styled by us at all beyond the icon itself. The script no longer
tries to work out what kind of row it is in — that detection is what produced three releases of
defects. Instead it adopts the idiom the row already uses, taken from
[Druidblack/jellyfin-icon-metadata](https://github.com/Druidblack/jellyfin-icon-metadata): neutralise
the anchor (`background: none`, `color: transparent`, `padding: 0`, `font-size: 0`) and draw the icon
in a `::before` with an explicit box. Size, horizontal gap and vertical offset are **measured** from
the row's own links — after
[n00bcodr/Jellyfin-Enhanced](https://github.com/n00bcodr/Jellyfin-Enhanced), which sizes its icons the
same way — and written as three CSS custom properties. Nothing else is written to the anchor, and
`test.js` check 11 is the guard on that. The caption stays in the DOM as `title`/`aria-label`, the one
dangling separator is dropped, and the link moves to the end of the row.
...
- **The client script must never key off jellyfin-web's own class names.** `Web/specialtomovie.js` once required its links to sit inside `#itemDetailPage:not(.hide) .itemExternalLinks`, and treated any anchor carrying text as proof the row was plain text. Both assumptions broke in the field in 1.0.17.0 and the enhancement failed **silently** - no badge, unreadable text on a dark theme, wrong position, and a first click that opened a new tab - because a selector that matches nothing looks exactly like a script that never loaded. Match on the plugin's own `stm=` marker. Anything that must survive a re-render - the in-app click, the icon, the position - is re-applied on every pass or handled in the capture phase, never set once at upgrade time.
- **Do not try to detect what kind of row it is, and do not copy a neighbour's geometry onto the anchor.** 1.0.18.0 and 1.0.19.0 both did, and both shipped a broken row: an icon that sat low, jumped sideways, drew a second icon inside itself, and grew on hover on themes the rest of the row does not. Both releases were pulled from GitHub and the manifest. What works is adopting the row's existing icon idiom and writing **only** three measured custom properties (`--stm-icon-size`, `--stm-icon-gap`, `--stm-icon-shift`) — `test.js` check 11 fails if anything else reaches the anchor's `style`. The full reasoning, including which `vertical-align` values were tried and what each measured, is in [`ARCHITECTURE.md`](ARCHITECTURE.md).
...
- **Comments in published source are lint-enforced.** `node agentic/tools/check-comments.mjs .` must print `clean` before a release, and everything outside `agentic/` is in scope, the config page's inline `<script>` included. A comment is a short note for the next line — two lines, two sentences, no "because"/"rather than"/"so that", no `///` XML docs, no `/* */` blocks. Rationale goes to [`ARCHITECTURE.md`](ARCHITECTURE.md) with the comment reduced to a pointer. When you take an explanation out of a comment, **move it**, don't delete it.
- `ARCHITECTURE.md`, `AUDIT.md`, `HANDOFF.md`, `IDEAS.md`, and `CLAUDE.md` live in `agentic/` **inside the repo**, so they are version controlled and are captured by release commits alongside code. That also makes them **public**: never write credentials, absolute local paths, hostnames, IP addresses, or anything else machine-specific into them. Their history before the move into the repo is not in `git log`.
