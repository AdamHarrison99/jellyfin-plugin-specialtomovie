...
- **Comments in published source are lint-enforced.** `node agentic/tools/check-comments.mjs .` must print `clean` before a release, and everything outside `agentic/` is in scope. A comment is a short note for the next line — two lines, two sentences, no "because"/"rather than"/"so that", no `///` XML docs, no `/* */` blocks. Rationale goes to [`ARCHITECTURE.md`](ARCHITECTURE.md) with the comment reduced to a pointer. When you take an explanation out of a comment, **move it**, don't delete it.
