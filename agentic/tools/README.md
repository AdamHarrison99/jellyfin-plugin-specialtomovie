...
| [`webclient-harness/`](webclient-harness/) | DOM harness for `Web/specialtomovie.js`: badge-row detection, contrast against the real backdrop, staying last in the row, and clicks staying in the app - including the first click on a freshly re-rendered anchor. Needs `npm install` in that directory once; `node_modules/` is gitignored and the lockfile pins the tree. | `node test.js` from `agentic/tools/webclient-harness` - prints PASS/FAIL per check; exit 0 = all passed, 1 = at least one failed. |
