...
| [`configpage-selects.js`](configpage-selects.js) | Checks that every `<select is="emby-select">` in `Configuration/configPage.html` sits in a positioned parent. Jellyfin draws each select's chevron in an absolutely positioned container, so a select whose parent is `static` loses its arrow to the page's top-right corner, where it reads as a stray control that does nothing — which is exactly what three filter selects did until each was wrapped. Needs no Jellyfin: it checks the containing block the page itself is responsible for. It drives a real browser through the `webclient-harness` install of `playwright-core`, so that directory needs its one-time `npm install` before this runs. | `node agentic/tools/configpage-selects.js` (from anywhere) (add a path to check another copy, and a second path to paste a console script in first; `STM_ARROW_MODE=after|end` picks where the simulated arrow is placed, and a console fix must pass under both). Exit 0 = every select holds its arrow, 1 = at least one would lose it, 2 = page unreadable, no browser, or `playwright-core` not installed. |
| [`configpage-arrow-fix.js`](configpage-arrow-fix.js) | Puts each stray `emby-select` chevron back on its own control from the browser console, on a server running a build whose config page does not wrap its selects. Wraps each one the way `configPage.html` now does, so the fix can be confirmed on a live settings page before the build carrying it ships. | Paste the file's contents into the DevTools console on the plugin's settings page; `__stmArrowFix.undo()` puts the DOM back, a reload does too. Checked against a released page with `node configpage-selects.js path/to/that/configPage.html configpage-arrow-fix.js`. |
