...
public class FullScanTask : IScheduledTask
{
    ...
    public string Description => "Scans all Season 0 episodes across configured TV libraries and looks up each against TMDB/TVDB. Catches episodes added before the plugin was installed, episodes missed during downtime, and previously failed lookups that may now succeed. Enforces the ignore list, processes force links, promotes DryRun pairs when dry run mode is disabled, and re-syncs watch state for all active pairs. In dry run mode, matches are detected and logged but no files are created.";
